  ...
  <div id="app">
      ...
      <el-form ref="form" class="unitForm" :model="form" id="form">
        ...
        <el-form-item label="录入人:">
          <el-select v-model="form.audit_person"  placeholder="请选择" clearable="true">
            <el-option
    ...
    <el-table
      ...
      <el-pagination
        ...
        :total="this.searchTableData.length"
      >
      </el-pagination>
  </div>
</template>
...
export default {
  ...
  methods: {
      getData() {
        axios.get("http://10.160.24.112:7000/KnowledgeAudit/AlreadyInit",).then(result => {
					this.searchTableData = result.data.data;
          this.searchTableData.sort(function(a,b){
            return a.audit_time < b.audit_time ? 1 : -1;
          })
				})
    },
    // 表单查询
    onSearch() {
      ...
      if(this.form.audit_person=="")
        this.form.audit_person=null;
      if(this.form.knowledge_type==null &&  this.form.severity_level==null && this.form.audit_person==null){
        this.getData();
      }
      else{
