  ...
  <div>
    <div class="table">
      ...
      <el-table :data="table1Data.slice((currentPage - 1) * pagesize, currentPage * pagesize)" id="table" stripe
        ...
        <el-table-column prop="dictId" label="字典主键" align="center" min-width="4%">
        </el-table-column>
        <el-table-column prop="dictName" label="字典名称" sortable align="center" min-width="10%">
        </el-table-column>
        <el-table-column prop="dictType" label="字典类型" sortable align="center" min-width="10%">
          <template slot-scope="scope">
            <div @click="cellClick(scope.row)" class="dictType">{{scope.row.dictType}}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="remark" label="备注" align="center" min-width="10%">
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" min-width="16%">
          <template slot-scope="scope">
            <div class="opr">
              ...
              <el-button type="primary" icon="el-icon-edit" size="small" @click="cellClick(scope.row)">列表</el-button>
...
export default {
  ...
  methods: {
    ...
    deleteSelect(data) {
      if (data) {
        ...
        this.$confirm("确认删除" + length + "行数据?", "提示", {
          ...
          .then(() => {
            this.$http({
              url: this.$http.adornUrl(`/system/dict/remove`),
              method: "POST",
    ...
    clearCache() {
      this.$http({
        url: this.$http.adornUrl(`/system/dict/clearCache`),
        method: "get",
        params: this.$http.adornParams({})
      }).then((res) => {
        if (res.data.code == 0) {
