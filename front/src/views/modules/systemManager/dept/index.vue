      ...
      <div ref="formDiv">
        <el-form ref="form" class="unitForm" :model="form" id="form">
          <el-form-item label="部门名称:" prop="deptName">
            <el-input placeholder="请输入" v-model="form.deptName" clearable> </el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$http({
        url: this.$http.adornUrl("/system/dept/list"),
        method: "post",
        ...
        if (res.data.length) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].parentId == parentId) {
              this.searchTableData.push(data[i]);
            }
            for (let j = 1; j < data.length; j++) {
              if (data[j].parentId == data[i].deptId) {
