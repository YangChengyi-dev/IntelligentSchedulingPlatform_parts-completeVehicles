...
def vehicle_init():
    ...
    return success_jsonify("Vehicle init successfully", json_data)
...
def explore():
    ...
    return success_jsonify("explore successfully", json_data)
...
def transport_init():
    ...
    return success_jsonify("transport init successfully", json_data)
...
def transport_search():
    ...
    return success_jsonify("transport search successfully", json_data)
...
def transport_cost():
    ...
    query_result = Tc.query(
        [Tc.id, Tc.site, Tc.history_sales, Tc.pro_sales, Tc.throughput, Tc.cost, Tc.o_throughput, Tc.o_cost,Tc.type, Tc.is_limit, Tc.method],
    ...
    write_file_path=os.path.join(BASE_DIR, f'data/sqlData/{req_type}品牌车运输成本{req_method}.xlsx')
    df = pd.DataFrame(json_data)
    df.columns = ['序列','分拨节点', "历史销量(年)", "对应省/市销量(年)", "吞吐量(年)","该省辐射的成本","优化吞吐量","优化辐射成本","类别","是否受限","方法"]
    columns =['分拨节点', "历史销量(年)", "对应省/市销量(年)", "吞吐量(年)","该省辐射的成本","优化吞吐量","优化辐射成本","类别","是否受限","方法"]
    subset = df[columns]
    # 替换空单元格
    ...
    return success_jsonify("transport cost successfully", {
        "all_data": json_data,
        "site": site_list,
...
def cost_download_excel():
    ...
    req_method = upload_data.get("method")
    # 生成Excel文件的路径
    filename = f"{req_type}品牌车运输成本{req_method}.xlsx"
    return downflie('data/sqlData', filename)
...
def select_empty_car():
    ...
    return success_jsonify("empty car search successfully", json_data)
...
def download_excel():
    ...
    return downflie('data/emptyCar', filename)
